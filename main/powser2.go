...
// ref: "go/test/chan/powser2.go"
//
// Power series package 2
//
...
func main() {
	...
	if len(os.Args) > 1 { // print
		...
		check(Add(Ones, Ones), itor(2), 500, "Add Ones Ones") // 1 1 1 1 1
		check(Add(Ones, Twos), itor(3), 500, "Add Ones Twos") // 3 3 3 3 3
		a := make([]*rat, N)
		d := Diff(Ones)
		for i := 0; i < N; i++ {
			a[i] = itor(int64(i + 1))
		...
		check(Sub(Ones, Twos), itor(-1), 10, "Sub Ones Twos") // -1 -1 -1 -1 -1
		m := Mul(Ones, Ones)
		for i := 0; i < N; i++ {
			a[i] = itor(int64(i + 1))
		}
		...
		print("Poly: ")
		Printn(po, 10)
		// check(po, itor(1), 3, "Poly") // TODO(apa): check fails: why?
